...
type (
	deps struct {
		...
		useCase           *create_link.CreateLinkUseCase
		id                uuid.UUID
...
func getDeps(t *testing.T) deps {
	...
	return deps{
		...
		useCase:           create_link.NewCreateLinkUseCase(repository, publisher, paymentProvider),
		id:                payment.ID,
		payment:           payment,
		link:              link,
...
func mockPublisher(t *testing.T, ctx context.Context, deps deps, response error) {
	t.Helper()
